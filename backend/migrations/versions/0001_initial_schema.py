...
Crée l'intégralité du schéma V1 (56 tables) en exécutant le script SQL de
référence versionné à part (db/schema.sql), afin de garantir une parité
exacte entre la documentation de conception et la base réellement créée.

Les migrations suivantes (ajout de colonnes, nouvelles tables...) pourront
être générées normalement via `alembic revision --autogenerate` une fois
une base de développement connectée, en s'appuyant sur les modèles
SQLAlchemy de app/models/.
...
def upgrade() -> None:
    """Crée toutes les extensions, types ENUM, tables et index de la V1."""
    op.execute(SCHEMA_SQL)


def downgrade() -> None:
    """Supprime toutes les tables (ordre inverse de création) puis les types
    ENUM. Les extensions (pgcrypto, citext, vector) ne sont volontairement
    pas désinstallées, d'autres objets de la base pouvant en dépendre."""
    op.execute("DROP TABLE IF EXISTS ai_messages CASCADE;")
    op.execute("DROP TABLE IF EXISTS ai_conversations CASCADE;")
    op.execute("DROP TABLE IF EXISTS notifications CASCADE;")
    op.execute("DROP TABLE IF EXISTS demo_views CASCADE;")
    op.execute("DROP TABLE IF EXISTS portfolio_evidence_skills CASCADE;")
    op.execute("DROP TABLE IF EXISTS portfolio_evidence CASCADE;")
    op.execute("DROP TABLE IF EXISTS lab_results CASCADE;")
    op.execute("DROP TABLE IF EXISTS quiz_attempt_answers CASCADE;")
    ...
    op.execute("DROP TABLE IF EXISTS user_certifications CASCADE;")
    op.execute("DROP TABLE IF EXISTS certification_requirements CASCADE;")
    op.execute("DROP TABLE IF EXISTS certifications CASCADE;")
    op.execute("DROP TABLE IF EXISTS knowledge_node_labs CASCADE;")
    op.execute("DROP TABLE IF EXISTS knowledge_node_demos CASCADE;")
    op.execute("DROP TABLE IF EXISTS knowledge_node_applications CASCADE;")
    op.execute("DROP TABLE IF EXISTS knowledge_node_used_in_lessons CASCADE;")
    op.execute("DROP TABLE IF EXISTS knowledge_node_dependencies CASCADE;")
    op.execute("DROP TABLE IF EXISTS knowledge_nodes CASCADE;")
    op.execute("DROP TABLE IF EXISTS governance_jurisdictions CASCADE;")
    op.execute("DROP TABLE IF EXISTS governance_standards CASCADE;")
    op.execute("DROP TABLE IF EXISTS glossary_terms CASCADE;")
    op.execute("DROP TABLE IF EXISTS resource_skills CASCADE;")
    op.execute("DROP TABLE IF EXISTS resource_lessons CASCADE;")
    op.execute("DROP TABLE IF EXISTS resource_courses CASCADE;")
    ...
    op.execute("DROP TABLE IF EXISTS question_banks CASCADE;")
    op.execute("DROP TABLE IF EXISTS lab_modes CASCADE;")
    op.execute("DROP TABLE IF EXISTS lab_skills CASCADE;")
    op.execute("DROP TABLE IF EXISTS labs CASCADE;")
    op.execute("DROP TABLE IF EXISTS lesson_depth_levels CASCADE;")
    op.execute("DROP TABLE IF EXISTS lesson_sections CASCADE;")
    op.execute("DROP TABLE IF EXISTS lesson_objectives CASCADE;")
    op.execute("DROP TABLE IF EXISTS lessons CASCADE;")
    op.execute("DROP TABLE IF EXISTS demos CASCADE;")
    op.execute("DROP TABLE IF EXISTS pathway_courses CASCADE;")
    op.execute("DROP TABLE IF EXISTS course_skills CASCADE;")
    op.execute("DROP TABLE IF EXISTS course_prerequisites CASCADE;")
    op.execute("DROP TABLE IF EXISTS courses CASCADE;")
    op.execute("DROP TABLE IF EXISTS pathway_skills CASCADE;")
    op.execute("DROP TABLE IF EXISTS pathway_prerequisites CASCADE;")
    op.execute("DROP TABLE IF EXISTS pathways CASCADE;")
    op.execute("DROP TABLE IF EXISTS user_profile_interest_skills CASCADE;")
    op.execute("DROP TABLE IF EXISTS skills CASCADE;")
    op.execute("DROP TABLE IF EXISTS schools CASCADE;")
    op.execute("DROP TABLE IF EXISTS user_profile_goals CASCADE;")
    op.execute("DROP TABLE IF EXISTS user_profiles CASCADE;")
    op.execute("DROP TABLE IF EXISTS goals CASCADE;")
    op.execute("DROP TABLE IF EXISTS learner_profile_types CASCADE;")
    op.execute("DROP TABLE IF EXISTS users CASCADE;")

    op.execute("DROP TYPE IF EXISTS ai_message_role CASCADE;")
    op.execute("DROP TYPE IF EXISTS notification_type CASCADE;")
    op.execute("DROP TYPE IF EXISTS lesson_progress_status CASCADE;")
    op.execute("DROP TYPE IF EXISTS user_certification_status CASCADE;")
    op.execute("DROP TYPE IF EXISTS certification_requirement_type CASCADE;")
    op.execute("DROP TYPE IF EXISTS quiz_kind CASCADE;")
    op.execute("DROP TYPE IF EXISTS lesson_depth_key CASCADE;")
